...
namespace TypeCobol.Compiler.SqlNodes.Common
{
    ...
    {
        protected List<NodeType> children_ = new List<NodeType>();

        public NodeType getChild(int i)
        {
            return hasChild(i) ? children_[i] : default(NodeType);
        }
        ...
        public void addChildren(List<NodeType> l)
        {
            children_.AddRange(l);
        }

        public bool hasChild(int i) { return children_.Count > i; }
        public void setChild(int index, NodeType n) { children_[index]=n; }
         ...
         */
        public  List<NodeType> getNodesPreOrder()
        {
            List<NodeType> result = new List<NodeType>();
            result.AddRange(getNodesPreOrderAux());
            return result;
        }

        protected List<NodeType> getNodesPreOrderAux()
        {
            List<NodeType> result = new List<NodeType>();
            result.Add(this);
         ...
         */
        public  List<NodeType> getNodesPostOrder()
        {
            List<NodeType> result = new List<NodeType>();
        ...
        protected  void getNodesPostOrderAux(
            List<NodeType> result)
        {
            foreach (NodeType child in children_) child.getNodesPostOrderAux(result);
            result.Add(this);
        }
        ...
        {
            int numNodes = 1;
            foreach (NodeType child in children_) numNodes += child.numNodes();
            return numNodes;
        }
         ...
         */
        public bool contains(NodeType node)
        {

...
namespace TypeCobol.Compiler.SqlNodes
{
  ...
  private bool isAnalyticFnCall_ = false;
        ...
        public bool isMergeAggFn() { return mergeAggInputFn_ != null; }
        ...
        public void setIsAnalyticFnCall(bool v) { isAnalyticFnCall_ = v; }
        public void setIsInternalFnCall(bool v) { isInternalFnCall_ = v; }
...
digitsAfter = Math.Min(digitsAfter, (int)scaleLiteral.getLongValue());
  ...
  protected override void analyzeImpl(Analyzer analyzer) 
{
        ...
        && children_.Count == 2)
    {
        ...
        children_.set(1, samplePerc.uncheckedCastTo(SqlNodeType.DOUBLE));
    }
    ...
    {
        (getFunctionNotFoundError(argTypes));
    }
    ...
    if (isAggregateFunction())
    {
      ...
      if (params_.isDistinct()) {
        ...
        if (fnName_.getFunction().Equals("group_concat", StringComparison.OrdinalIgnoreCase)
            ...
            ("Second parameter in GROUP_CONCAT(DISTINCT)" +
    ...
    if (params_.isIgnoreNulls() && !isAnalyticFnCall_) {
      ("Function " + fnName_.getFunction().ToUpperInvariant()
